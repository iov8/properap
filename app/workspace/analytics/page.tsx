...
export default async function AnalyticsPage({ searchParams }:{ searchParams:Promise<{tab?:string;section?:string;range?:string}> }) {
  ...
  const renderSection = () => {
    if (section === "dashboard") return <><Metrics items={[["Listing views",views.length,"Across the selected period"],["Unique visitors",unique,"Anonymous first-party visitors"],["Inquiries",inquiryRows.length,percent(inquiryRows.length, views.length)+" view-to-inquiry rate"],["Website visits",siteViews,"Visits to your public website"]]} /><div className="analytics-two-column"><section><span>Top listings</span><h2>What attracts attention</h2>{bars(perListing.slice(0,6).map((row) => ({label:row.title,value:row.views})))}</section><section><span>Audience</span><h2>Where visitors are located</h2>{bars(countries.slice(0,6))}</section></div></>;
