...
function referral(inputHost?: string) { if (!inputHost) return { host: null, channel: "direct" }; try { const host = inputHost.toLowerCase().replace(/^www\./, ""); if (host === "properap.com" || host.endsWith(".properap.com")) return { host, channel: "properap" }; if (/google|bing|yahoo|duckduckgo/.test(host)) return { host, channel: "search" }; if (/facebook|instagram|linkedin|tiktok|youtube|snapchat/.test(host)) return { host, channel: "social" }; return { host: /^[a-z0-9.-]+$/.test(host) ? host.slice(0, 253) : null, channel: "referral" }; } catch { return { host: null, channel: "unknown" }; } }
...
export async function POST(request: NextRequest) {
  ...
  const metadata = Object.fromEntries(Object.entries(parsed.data.metadata ?? {}).filter(([key]) => allowedMetadata.has(key))); const country = request.headers.get("x-vercel-ip-country")?.toUpperCase(); const ref = referral(parsed.data.referrerHost);
