...
function SiteShell({ site, listings, media, assets, testimonials }: { site: NonNullable<Awaited<ReturnType<typeof getProfessionalSite>>>; listings: SiteListing[]; media: SiteMedia[]; assets: {id:string;placement:string}[]; testimonials: Testimonial[] }) {
  ...
  const heroTextStyle = { color: palette.text, WebkitTextFillColor: palette.text } as CSSProperties;
  ...
  const section = {
    hero: <section key="hero" className={`professional-site-hero ${site.site_type === "brokerage" ? "brokerage-site-hero" : "agent-site-hero"}`} style={{ backgroundColor: palette.primary, color: palette.text }}><svg className="professional-site-hero-surface" aria-hidden="true" viewBox="0 0 100 100" preserveAspectRatio="none"><rect width="100" height="100" fill={palette.primary} /></svg><div className="professional-site-hero-content">{asset ? <Image className={site.site_type === "brokerage" ? "site-logo" : "site-profile-photo"} src={`/media/listings/${asset.id}/card.webp?v=${asset.id}`} alt={`${site.display_name} ${site.site_type === "brokerage" ? "logo" : "profile photograph"}`} width={220} height={220} unoptimized /> : null}<div className="professional-site-hero-copy">{site.site_type === "brokerage" ? <AutoFitHeading className="brokerage-site-title"><span style={heroTextStyle}>{site.display_name}</span></AutoFitHeading> : <h1><span style={heroTextStyle}>{site.display_name}</span></h1>}<p><span style={heroTextStyle}>{site.headline ?? (site.site_type === "agent" ? "Local property guidance with clear, professional service." : "Verified property opportunities across Jamaica.")}</span></p>{site.site_type === "brokerage" ? <Link className="site-join-button" href={`/join/${site.slug}`}><span style={heroTextStyle}>Join this brokerage</span></Link> : null}</div></div></section>,
